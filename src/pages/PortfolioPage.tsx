...
function PositionCard({ pos, onManage }: { pos: ProtocolPosition; onManage: () => void }) {
  ...
  const borderColor = hasBorrow ? "rgba(251,191,36,0.25)" : "rgba(67,72,78,0.3)";
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: "#a7abb2", fontFamily: "Inter, sans-serif", marginTop: 2 }}>
            {pos.asset} · {pos.chain}
          </div>
        </div>
      ...
      {hasSupply && (
        <div style={{
          ...
          background: "#141a20", borderRadius: 8, padding: "9px 12px", marginBottom: hasBorrow ? 6 : 14,
        }}>
          <div>
            <div style={{ fontSize: 10, color: "#a7abb2", fontFamily: "Inter, sans-serif", marginBottom: 2, textTransform: "uppercase", letterSpacing: "0.07em" }}>
              Supplied
            </div>
      ...
      {hasBorrow && (
        <div style={{
          display: "flex", alignItems: "center", justifyContent: "space-between",
          background: "#141a20", borderRadius: 8, padding: "9px 12px", marginBottom: 14,
        }}>
          <div>
            <div style={{ fontSize: 10, color: "#a7abb2", fontFamily: "Inter, sans-serif", marginBottom: 2, textTransform: "uppercase", letterSpacing: "0.07em" }}>
              Borrowed
      ...
      <button onClick={onManage} style={{
        ...
        background: "rgba(255,255,255,0.04)", color: "#a7abb2",
        fontSize: 11, fontWeight: 700, fontFamily: "Inter, sans-serif",
        transition: "background 150ms",
      }}
        onMouseEnter={e => (e.currentTarget.style.background = "rgba(134,239,172,0.06)")}
...
function LoadingCard() {
  return (
    <div style={{
      background: "#0e1419", border: "1px solid rgba(67,72,78,0.2)",
      borderRadius: 12, padding: "16px 18px", minHeight: 130,
      display: "flex", alignItems: "center", justifyContent: "center",
    }}>
      <span style={{ fontSize: 12, color: "#a7abb2", fontFamily: "Inter, sans-serif" }}>
        Fetching positions…
      </span>
    </div>
  );
...
function ConnectedPortfolio() {
  ...
  const protocolCount = new Set(supplyPositions.concat(borrowPositions).map(p => `${p.protocolKey}:${p.chainId}`)).size;
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
          ...
          <SummaryCard
            ...
            sub={netPosition > 0 ? `Net ${formatUSD(netPosition)}` : "Ethereum & Base"}
          />
        </div>
      </div>
      ...
      {!isLoading && (
        <div style={{
          ...
          {["Aave V3", "Compound V3", "Morpho Blue", "Spark", "Moonwell", "Seamless", "Silo", "Euler", "Granary"].map(p => (
            <span key={p} style={{
              fontSize: 10, fontWeight: 600, color: "#eaeef5",
          ...
          <span style={{ fontSize: 10, color: "#a7abb2", fontFamily: "Inter, sans-serif", marginLeft: "auto" }}>
            Ethereum · Base
          </span>
        </div>
      )}
